...
void main() {
  ...
  test(
    'recordBudgetPurchase creates an entry with the correct snapshot and total',
    () {
      final snapshot = [
  ...
  test(
    'recordBudgetPurchase inserts the newest entry at the head of the list',
    () {
  ...
  test(
    'recordBudgetPurchase with an empty snapshot does not create an entry',
    () {
  ...
  test('deleteBudgetHistoryEntry removes only the matching entry', () {
    ...
    final toDelete = notifier.budgetHistory.firstWhere(
      (e) => e.items.single.name == '删除',
    );
