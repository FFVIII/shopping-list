...
Category _category(String id, {String shelfZone = '其他', int defaultDays = 7}) {
  return Category(
    id: id,
    name: id,
...
void main() {
  ...
  group('applyPurchase (完成购物 → 入库存)', () {
    ...
    test('creates a new inventory entry when no match exists', () {
      final result = applyPurchase(
        inventory: const [],
        item: _shoppingItem('s1', category: cat, name: '牛奶', quantityLabel: '2盒'),
        estimatedDays: 7,
        newId: 'inv_new',
        now: now,
      );
    ...
    test('updates the matching existing entry in place instead of duplicating', () {
      final existing = _inventoryItem('inv_1',
          category: cat,
          name: '牛奶',
    ...
    test('two same-named inventory rows: sourceInventoryId picks the right one', () {
      ...
      // first.
      final rowA = _inventoryItem('inv_a',
          category: cat,
          name: '牛奶',
          purchasedAt: DateTime(2026, 1, 1),
          estimatedDays: 3);
      final rowB = _inventoryItem('inv_b',
          category: cat,
          name: '牛奶',
          purchasedAt: DateTime(2026, 1, 1),
          estimatedDays: 3);
      ...
      final result = applyPurchase(
        inventory: [rowA, rowB],
        item: _shoppingItem('s1',
            category: cat, name: '牛奶', sourceInventoryId: 'inv_b'),
        estimatedDays: 9,
        newId: 'unused',
        now: now,
      );
      ...
      expect(result.firstWhere((i) => i.id == 'inv_a').estimatedDays, 3); // untouched
      expect(result.firstWhere((i) => i.id == 'inv_b').estimatedDays, 9); // updated
    });
    ...
    test('falls back to name matching when sourceInventoryId is absent', () {
      ...
      final result = applyPurchase(
        inventory: [existing],
        item: _shoppingItem('s1', category: cat, name: '鸡蛋'), // manually typed, no source
        estimatedDays: 9,
        newId: 'unused',
        now: now,
      );
    ...
    test('preserves the existing shelfCode when the shopping item has none', () {
      final existing = _inventoryItem('inv_1',
          category: cat,
          name: '牛奶',
          purchasedAt: DateTime(2026, 1, 1),
      ...
      final result = applyPurchase(
        inventory: [existing],
        item: _shoppingItem('s1',
            category: cat, name: '牛奶', sourceInventoryId: 'inv_1'), // no shelfCode
        estimatedDays: 7,
        newId: 'unused',
        now: now,
      );
  ...
  group('reassignCategoryToFallback (删除分类后重新指派)', () {
    test('reassigns matching items across all three lists, leaves others untouched', () {
